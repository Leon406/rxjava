...
import rx0140.util.Closings;
import rx0140.util.Opening;
import rx0140.util.Openings;
...
public final class OperationWindow extends ChunkedOperation {
     ...
     * values from the specified {@link rx0140.Observable} source and stores them in a window until the {@link rx0140.Observable} constructed using the {@link Func0} argument, produces a
     * {@link Closing} value. The window is then
     * emitted, and a new window is created to replace it. A new {@link rx0140.Observable} will be constructed using the
     * provided {@link Func0} object, which will determine when this new window is emitted. When the source {@link rx0140.Observable} completes or produces an error, the current window
     * is emitted, and the event is propagated
     * to all subscribed {@link rx0140.Observer}s.</p>
     * 
     ...
     * 
     * @param source
     *            The {@link rx0140.Observable} which produces values.
     * @param windowClosingSelector
     *            A {@link Func0} object which produces {@link rx0140.Observable}s. These {@link rx0140.Observable}s determine when a window is emitted and replaced by simply
     *            producing an {@link Closing} object.
     * @return
    ...
    /**
     * <p>This method creates a {@link Func1} object which represents the window operation. This operation takes
     * values from the specified {@link rx0140.Observable} source and stores them in the currently active window. Initially
     * there are no windows active.</p>
     * 
     * <p>Windows can be created by pushing a {@link Opening} value to the "windowOpenings" {@link rx0140.Observable}.
     * This creates a new window which will then start recording values which are produced by the "source" {@link rx0140.Observable}. Additionally the "windowClosingSelector" will be used to construct an
     * {@link rx0140.Observable} which can produce {@link Closing} values. When it does so it will close this (and only this) newly created
     * window. When the source {@link rx0140.Observable} completes or produces an error, all windows are emitted, and the
     * event is propagated to all subscribed {@link rx0140.Observer}s.</p>
     * 
     ...
     *            An {@link rx0140.Observable} which when it produces a {@link Opening} value will
     *            create a new window which instantly starts recording the "source" {@link rx0140.Observable}.
     * @param windowClosingSelector
     *            A {@link Func0} object which produces {@link rx0140.Observable}s. These {@link rx0140.Observable}s determine when a window is emitted and replaced by simply
     *            producing an {@link Closing} object.
     * @return
    ...
    /**
     * <p>This method creates a {@link Func1} object which represents the window operation. This operation takes
     * values from the specified {@link rx0140.Observable} source and stores them in a window until the window contains
     * a specified number of elements. The window is then emitted, and a new window is created to replace it.
     * When the source {@link rx0140.Observable} completes or produces an error, the current window is emitted, and
     * the event is propagated to all subscribed {@link rx0140.Observer}s.</p>
     * 
     * <p>Note that this operation only produces <strong>non-overlapping windows</strong>. At all times there is
     * exactly one window actively storing values.</p>
     * 
    ...
    /**
     * <p>This method creates a {@link Func1} object which represents the window operation. This operation takes
     * values from the specified {@link rx0140.Observable} source and stores them in all active windows until the window
     * contains a specified number of elements. The window is then emitted. windows are created after a certain
     * amount of values have been received. When the source {@link rx0140.Observable} completes or produces an error, the
     * currently active windows are emitted, and the event is propagated to all subscribed {@link rx0140.Observer}s.</p>
     * 
     ...
     *            that this is the same as calling {@link OperationWindow#window(rx0140.Observable, int)}.
    ...
    /**
     * <p>This method creates a {@link Func1} object which represents the window operation. This operation takes
     * values from the specified {@link rx0140.Observable} source and stores them in a window. Periodically the window
     ...
     *            The {@link TimeUnit} defining the unit of time for the timespan.
     * @param scheduler
     *            The {@link rx0140.Scheduler} to use for timing windows.
     * @return
     *         the {@link Func1} object representing the specified window operation.
     */
