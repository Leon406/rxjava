...
import rx100.exceptions.CompositeException;
import rx100.exceptions.Exceptions;
import rx100.functions.Action1;
import rx100.functions.Func1;
import rx100.internal.operators.NotificationLite;
import rx100.internal.util.UtilityFunctions;
import rx100.schedulers.Timestamped;
import rx100.subjects.ReplaySubject.NodeList.Node;
import rx100.subjects.SubjectSubscriptionManager.SubjectObserver;
...
public final class ReplaySubject<T> extends Subject<T, T> {
    ...
    static final class BoundedState<T> implements ReplayState<T, Node<Object>> {
        ...
        @Override
        public void replayObserver(SubjectObserver<? super T> observer) {
            Node<Object> lastEmittedLink = observer.index();
            Node<Object> l = replayObserverFromIndex(lastEmittedLink, observer);
            observer.index(l);
        }

        @Override
        public Node<Object> replayObserverFromIndex(
                Node<Object> l, SubjectObserver<? super T> observer) {
            while (l != tail()) {
        ...
        @Override
        public Node<Object> replayObserverFromIndexTest(
                Node<Object> l, SubjectObserver<? super T> observer, long now) {
            while (l != tail()) {
    ...
    static final class DefaultOnAdd<T> implements Action1<SubjectObserver<T>> {
        ...
        @Override
        public void call(SubjectObserver<T> t1) {
            Node<Object> l = state.replayObserverFromIndex(state.head(), t1);
            t1.index(l);
        }
