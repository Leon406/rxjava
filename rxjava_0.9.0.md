...
#### 此篇不在介绍整体调用流程,请参考上篇rxjava\_0.5.0, 会针对某几个模块进行解析
...
![1560309346361](https://github.com/Leon406/rxjava/tree/a294fae472d178a748cd400c64612a4eaaf3a851/img/1560309346361.png)
...
* AsyncSubject
  ...
  当complete的时候,发送最后一个事件,_源码see@ AsyncSubject\#onCompleted_
...
* BehaviorSubject
  ...
  _源码see@ BehaviorSubject\#\#onSubscribe, 订阅时会发送上一个value 的next 事件_
...
​ _比较简单,源码see@ PublishSubject\#onNext, 订阅后,才会收到事件_
...
* ReplaySubject
  ...
  _源码see@ReplaySubject\#onNext 订阅后,会从history 遍历发送事件_

### 常用operator解析  ---主要看Fun1 这个类的call 方法及对应Observer

* 创建 \(不多做介绍 see@ rxjava\_0.5.0.md\)
  * create      创建  最原始的
  * just           单个数据转 Observable\#toObservable
  * from        多个数据转 Observable\#toObservable
* 变换
  * map                              变换 1对1    \(不多做介绍 see@ rxjava\_0.5.0.md\)
  * flatMap                        1对多
  * scan                              累计
  * groupBy                       分组
* 过滤
...
* 组合
  ...
  * zip                                   压缩成Pair
* 转换 
  * toObservable        转Observable\(不多做介绍 see@ rxjava\_0.5.0.md\)
...
#### mapMany 与flatMap等价  注意  Func1&gt; func 返回是Observable
